// Background Music Player for AB Autoz
(function() {
    var speakerOn = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#d4af37" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><path d="M19.07 4.93a10 10 0 0 1 0 14.14"></path><path d="M15.54 8.46a5 5 0 0 1 0 7.08"></path></svg>';
    var speakerOff = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#d4af37" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><line x1="23" y1="9" x2="17" y2="15"></line><line x1="17" y1="9" x2="23" y2="15"></line></svg>';

    var audio = document.createElement('audio');
    audio.id = 'bg-music';
    audio.loop = true;
    audio.volume = 0.3;
    audio.src = 'audio/beat.mp4';
    document.body.appendChild(audio);

    var btn = document.createElement('button');
    btn.id = 'music-toggle';
    btn.innerHTML = speakerOff;
    btn.title = 'Toggle Music';
    btn.style.cssText = 'position:fixed;bottom:30px;right:30px;z-index:99999;width:55px;height:55px;border-radius:50%;border:2px solid #d4af37;background:rgba(0,0,0,0.9);color:#d4af37;font-size:24px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all 0.3s ease;box-shadow:0 4px 15px rgba(212,175,55,0.4);';
    btn.onmouseenter = function() { this.style.background='rgba(212,175,55,0.2)'; this.style.transform='scale(1.1)'; this.style.boxShadow='0 4px 20px rgba(212,175,55,0.6)'; };
    btn.onmouseleave = function() { this.style.background='rgba(0,0,0,0.9)'; this.style.transform='scale(1)'; this.style.boxShadow='0 4px 15px rgba(212,175,55,0.4)'; };
    document.body.appendChild(btn);

    var isPlaying = false;

    function tryPlay() {
        audio.play().then(function() {
            isPlaying = true;
            btn.innerHTML = speakerOn;
        }).catch(function() {
            isPlaying = false;
            btn.innerHTML = speakerOff;
        });
    }

    btn.addEventListener('click', function() {
        if (isPlaying) {
            audio.pause();
            isPlaying = false;
            btn.innerHTML = speakerOff;
        } else {
            audio.play().then(function() {
                isPlaying = true;
                btn.innerHTML = speakerOn;
            });
        }
    });

    if (document.readyState === 'complete' || document.readyState === 'interactive') {
        setTimeout(tryPlay, 500);
    } else {
        document.addEventListener('DOMContentLoaded', function() { setTimeout(tryPlay, 500); });
    }

    document.addEventListener('click', function initPlay(e) {
        if (e.target === btn || btn.contains(e.target)) return;
        if (!isPlaying) { tryPlay(); }
        document.removeEventListener('click', initPlay);
    }, {once: false});
})();
